...
CFloatImage ConvolveKernel_1331;
...
KernelInit::KernelInit()
{
    ...
    static float k_1331[4] = {0.125f, 0.375f, 0.375f, 0.125f};
    ...
    ConvolveKernel_1331.ReAllocate(CShape(4, 1, 1), k_1331, false, 4);
    ConvolveKernel_1331.origin[0] = -1;
    ConvolveKernel_14641.ReAllocate(CShape(5, 1, 1), k_14641, false, 5);
    ConvolveKernel_14641.origin[0] = -1; // used to be -2, but since it uses first pixel when 
	// downsampling by 3, need to make first pixel origin also
    ConvolveKernel_8TapLowPass.ReAllocate(CShape(8, 1, 1), k_8ptI, false, 8);
    ConvolveKernel_8TapLowPass.origin[0] = -3; // used to be -4,
		// but -3 gives better alignment, at least when downsampling by factor of 2
}
